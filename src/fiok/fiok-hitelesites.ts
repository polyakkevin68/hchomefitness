...
export async function hashFiokJelszot(jelszo: string): Promise<string> {
...
export async function ellenorizFiokJelszot(jelszo: string, tarolt: string): Promise<boolean> {
...
export function ujFiokToken(): string {
  return randomBytes(32).toString("base64url");
}

export function hashFiokToken(token: string): string {
