  ...
  return (
    <main>
      ...
      <header className="site-header">
        ...
        <nav aria-label="Fő navigáció"><a href="#katalogus">Katalógus</a><a href="#rolunk">Rólunk</a><a href="#kapcsolat">Kapcsolat</a></nav>
      ...
      <section className="category-band" aria-label="Kategóriák">
        <span className="band-label">FELFEDEZÉS</span>{(categories.length ? categories.slice(0, 4) : ["Futópadok", "Szobakerékpárok", "Erőgépek", "Kiegészítők"]).map((category) => <a key={category} href="#katalogus">{category}<span>↗</span></a>)}
      </section>
      ...
      <section className="catalog-section" id="katalogus">
        ...
        {catalog.osszesTermekSzama === 0 ? <div className="empty-state"><span className="empty-symbol">○</span><h3>A katalógus jelenleg nem elérhető</h3><p>A termékforrás beállítása után itt jelennek meg az ellenőrzött HC Home Fitness termékek.</p></div> : <>
          {isUnasPreview ? <div className="demo-notice"><strong>Fejlesztői előnézet</strong> — az UNAS bruttó ár HUF-ban, a tulajdonos jóváhagyása szerint. A termékek még nincsenek közzétéve, vásárolni nem lehet.</div> : process.env.NODE_ENV !== "production" && <div className="demo-notice"><strong>Fejlesztői minta</strong> — ezek tesztadatok, nem valódi termékek.</div>}
      ...
      <footer id="kapcsolat"><Link className="brand footer-brand" href="/"><span className="brand-mark">HC</span><span>HOME FITNESS</span></Link><span>© 2026 HC Home Fitness</span><span>Kapcsolat és jogi tájékoztatók hamarosan</span></footer>
    </main>
  );
}
