  ...
  async function igenytKuldes(esemeny: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!alap.ajanlatToken) {
        const ajanlatValasz = await fetch("/api/penztar/ajanlat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ verzio: kosar.verzio, ...(kuponKod.trim() ? { kuponKod: kuponKod.trim() } : {}) }) });
