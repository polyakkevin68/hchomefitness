...
type Rendeles = { id: string; publicId: string; status: string; customerName: string; customerEmail: string; customerPhone: string; addressSnapshot: { szallitasiCim?: { iranyitoszam?: string; telepules?: string; cim?: string } }; itemSnapshots: Tetel[]; totalHuf: number; createdAt: string; shipment: Szallitas | null };
...
export default function RendelesKezelo() {
  ...
  return <div>{!belepve ? <form className="muveleti-urlap" onSubmit={(esemeny) => void belepes(esemeny)}><label>E-mail-cím<input type="email" autoComplete="username" value={email} onChange={(esemeny) => setEmail(esemeny.target.value)} required maxLength={254} /></label><label>Jelszó<input type="password" autoComplete="current-password" value={jelszo} onChange={(esemeny) => setJelszo(esemeny.target.value)} required maxLength={256} /></label><button type="submit" disabled={betoltes}>{betoltes ? "Belépés…" : "Belépés a kezelőfelületre"}</button></form> : <><button type="button" disabled={betoltes} onClick={() => void listaBetoltes()}>Rendelések frissítése</button><button type="button" disabled={betoltes} onClick={() => void kilepes()}>Kijelentkezés</button>{rendelesek.length === 0 && <p>Nincs megjelenített kezelendő rendelés.</p>}</>}{hiba && <p className="muveleti-hiba" role="alert">{hiba}</p>}{rendelesek.map((rendeles) => <article className="muveleti-kartya" key={rendeles.id}><p className="eyebrow">{rendeles.publicId} · {new Date(rendeles.createdAt).toLocaleString("hu-HU")}</p><h2>{rendeles.customerName}</h2><p>{rendeles.customerEmail}<br />{rendeles.customerPhone}</p><p>{rendeles.addressSnapshot.szallitasiCim?.iranyitoszam} {rendeles.addressSnapshot.szallitasiCim?.telepules}, {rendeles.addressSnapshot.szallitasiCim?.cim}</p><ul>{rendeles.itemSnapshots.map((tetel, index) => <li key={`${tetel.cikkszam}-${index}`}>{tetel.nev} · {tetel.mennyiseg} db · {huf.format(tetel.sorOsszegHuf ?? 0)}</li>)}</ul><strong>Összesen: {huf.format(rendeles.totalHuf)}</strong>{rendeles.status === "PENDING_CONFIRMATION" ? <><label className="muveleti-urlap">Ellenőrzés eredménye<textarea minLength={10} maxLength={500} required value={megjegyzesek[rendeles.publicId] ?? ""} onChange={(esemeny) => setMegjegyzesek((elozo) => ({ ...elozo, [rendeles.publicId]: esemeny.target.value }))} placeholder="Például: készlet kézzel ellenőrizve" /></label><div className="muveleti-gombok"><button type="button" disabled={betoltes} onClick={() => void keszletDontes(rendeles.publicId, "megerosit")}>Készlet visszaigazolása</button><button type="button" disabled={betoltes} onClick={() => void keszletDontes(rendeles.publicId, "elutasit")}>Nem elérhető</button></div></> : <><p>Teljesítés: {rendeles.shipment?.state ?? "Előkészítésre vár"}</p>{!rendeles.shipment || rendeles.shipment.state === "PENDING" ? <button type="button" disabled={betoltes} onClick={() => void frissitSzallitast(rendeles.publicId, "PROCESSING")}>Teljesítés elkezdése</button> : null}{rendeles.shipment?.state === "PROCESSING" && <><label>Futárszolgálat<input value={szallitasok[rendeles.publicId]?.futar ?? rendeles.shipment.carrier ?? ""} onChange={(e) => setSzallitasok((elozo) => ({ ...elozo, [rendeles.publicId]: { ...elozo[rendeles.publicId], futar: e.target.value, kovetesiSzam: elozo[rendeles.publicId]?.kovetesiSzam ?? rendeles.shipment?.trackingNumber ?? "", kovetesiUrl: elozo[rendeles.publicId]?.kovetesiUrl ?? rendeles.shipment?.trackingUrl ?? "" } }))} /></label><label>Követési szám<input value={szallitasok[rendeles.publicId]?.kovetesiSzam ?? rendeles.shipment.trackingNumber ?? ""} onChange={(e) => setSzallitasok((elozo) => ({ ...elozo, [rendeles.publicId]: { ...elozo[rendeles.publicId], futar: elozo[rendeles.publicId]?.futar ?? rendeles.shipment?.carrier ?? "", kovetesiSzam: e.target.value, kovetesiUrl: elozo[rendeles.publicId]?.kovetesiUrl ?? rendeles.shipment?.trackingUrl ?? "" } }))} /></label><label>Követési hivatkozás<input type="url" value={szallitasok[rendeles.publicId]?.kovetesiUrl ?? rendeles.shipment.trackingUrl ?? ""} onChange={(e) => setSzallitasok((elozo) => ({ ...elozo, [rendeles.publicId]: { ...elozo[rendeles.publicId], futar: elozo[rendeles.publicId]?.futar ?? rendeles.shipment?.carrier ?? "", kovetesiSzam: elozo[rendeles.publicId]?.kovetesiSzam ?? rendeles.shipment?.trackingNumber ?? "", kovetesiUrl: e.target.value } }))} /></label><button type="button" disabled={betoltes} onClick={() => void frissitSzallitast(rendeles.publicId, "SHIPPED")}>Feladás rögzítése</button></>}{rendeles.shipment?.state === "SHIPPED" && <><p>{rendeles.shipment.carrier}: {rendeles.shipment.trackingNumber}</p><button type="button" disabled={betoltes} onClick={() => void frissitSzallitast(rendeles.publicId, "DELIVERED")}>Kézbesítés rögzítése</button></>}</>}</article>)}</div>;
}
