...
const tarolo = vi.hoisted(() => ({ rogzitRendelesiIgenyt: vi.fn() }));
...
function keres(body: unknown, session = munkamenet, origin = "http://localhost:3000") {
  const headers = new Headers({ origin, cookie: `hc_kosar=${session}`, "content-type": "application/json" });
...
describe("rendelési igény API útvonal", () => {
  ...
  it("a szerver rögzíti a kérést, hozzáférési sütit ad, de tokent nem küld a böngészőnek", async () => {
    ...
    expect(tarolo.rogzitRendelesiIgenyt).toHaveBeenCalledWith(munkamenet, kereses);
  });
});
