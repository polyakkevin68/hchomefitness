...
| Worker leállás | CI-jelpróba hozzáadva; eredménye várható | Két egységteszt igazolja a kíméletes leállítási sorrendet és a hibaágat. A Windowsos futtatóból küldött megszakítás nem adott `worker.stopped` naplót. A Linuxos PostgreSQL CI most valódi workerfolyamatot indít, SIGTERM jelet küld, és ellenőrzi a leállási eseményt; az új futás még folyamatban van. |
| CI adatbázis-környezet | Sikeres távoli futás | GitHub Actions 36127162898: PostgreSQL 17, migráció, lint, típusellenőrzés, 124 teszt és production build mind sikeres. Az első futásban a rendelési próba a helyi jogidokumentum-verziók hiánya miatt bukott; a workflow ezeket csak a tesztlépésnek adja át. A SIGTERM-próba külön, új commit utáni futásban ellenőrzendő. |
