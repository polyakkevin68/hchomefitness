...
- A GitHub Actions munkafolyamat PostgreSQL 17-et indít, migrációkat alkalmaz, majd lintet, típusellenőrzést, teszteket és buildet futtat. Az első helyi commit elkészült (`b84a23c`), a `main` ág HTTPS-távoli címe a képernyőképen látható GitHub-tárhelyre mutat. A `.env` kimaradt; a feltöltést a GitHub „Repository not found” hibával elutasította, ezért a távoli CI nem indult el. A cím vagy hozzáférés tisztázása szükséges.
...
- A worker leállítása külön függvénybe került: előbb kíméletes leállítást kér, hiba esetén kényszerített lezárást próbál, majd bontja az adatbázis-kapcsolatot. Két egységteszt ellenőrzi a sorrendet és mindkét ágat; a Windowsos futófolyamattól továbbra sem sikerült `worker.stopped` eseményt megfigyelni.
...
- Worker-próba: induláskor megjelent a `worker.started`; a Windowsos futtató megszakításakor nem kaptam vissza `worker.stopped` naplóbejegyzést, ezért a szabályos leállás nem tekinthető igazoltnak.
...
- Nyitott kapuk: eladó/jogi tartalom, mobil és billentyűzetes ellenőrzés, éles forrás teljes és részleges hibapróbája, SimplePay sandbox, Számlázz.hu/MailerSend kapcsolat, worker valódi jelre történő leállása és riasztás, staging domain/TLS és hozzáférések. Emiatt M8 nem zárható le.
...
Az M8 további feladata a staging és a külső szolgáltatói próbák hozzáférés utáni ellenőrzése, a jogi, hozzáférhetőségi és üzemeltetési kapuk igazolása, valamint a worker valódi jelre történő leállásának bizonyítása. A helyi PostgreSQL mentés-visszaállítása és az API 1000 termék/50 kliens/10 perces terhelési próbája sikeres (p95 213 ms). A helyi prototípus a `http://localhost:3000` címen próbálható ki. Az M6 nyitva marad; M7-ben a Számlázz.hu és MailerSend integrációjára még szükség van. A teljes csomag 32 tesztfájl/124 teszt, lint, típusellenőrzés, Prisma-migrációs állapot és production build sikeres; külső szolgáltatói művelet nem történt.
